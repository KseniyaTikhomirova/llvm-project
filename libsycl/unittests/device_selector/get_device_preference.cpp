...
TEST(DeviceSelector, AspectSelector) {
  ...
  const std::vector<sycl::aspect> GpuAspect{sycl::aspect::gpu};
  ...
  auto RequireGpuSelector = sycl::aspect_selector(GpuAspect, EmptyAspects);
  EXPECT_EQ(RequireGpuSelector(Dev), 1050);

  auto DenyGpuSelector = sycl::aspect_selector(EmptyAspects, GpuAspect);

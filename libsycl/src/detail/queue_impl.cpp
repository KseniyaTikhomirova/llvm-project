...
static void setKernelLaunchArgs(const detail::UnifiedRangeView &Range, ol_kernel_launch_size_args_t &ArgsToSet)
{
  ...
  QueueImpl::QueueImpl(DeviceImpl &deviceImpl,
...
static bool checkEventsPlatformMatch(std::vector<EventImplPtr> & Events, const PlatformImpl& QueuePlatform)
{ 
   // liboffload limitation to olWaitEvents. We can't do any extra handling for cross context/platform events without host task support now.
   //   "The input events can be from any queue on any device provided by the
   //   same platform as `Queue`."
   return std::all_of(Events.cbegin(), Events.cend(), [&QueuePlatform](const EventImplPtr& Event) { return &Event->getPlatformImpl() == &QueuePlatform; } );
 }
...
void QueueImpl::setKernelParameters(std::vector<EventImplPtr> &&Events,
  ...
  if (!checkEventsPlatformMatch(Events, MDevice.getPlatformImpl()))
    throw sycl::exception(sycl::make_error_code(sycl::errc::feature_not_supported),
                          "libsycl doesn't support cross-context/platform event dependencies now.");

  // TODO: this convertion and storing only offload events is possible only while we
  // don't have host tasks (and features based on host tasks, like streams).
...
void QueueImpl::submitKernelImpl(const char *KernelName,
  ...
  if (isFailed(Result))
    throw sycl::exception(sycl::make_error_code(sycl::errc::runtime),
                         std::string("Kernel submission (") + KernelName + ") failed with " + formatCodeString(Result));
  ...
  void QueueImpl::wait() {
    callAndThrow(olSyncQueue, MOffloadQueue);
  }

...
private:
  ...
  /// \param Device a device that device image must be compatible with.
  ...
  /// This call must be protected with mutex since it updates MPrograms and MProgramWrappers collections.
  ...
  /// This call must be protected with mutex since it updates MKernels collection.
  ...
  /// \param Device a device that kernel must be created with.
  /// \return liboffload kernel for the requested configuration or nullptr if such kernel is not found.
  ...
  // All data collections, created from data in __sycl_register_lib, must be protected with this mutex.
  // Protects data that can be modified only by modules load/unload.
  ...
  std::unordered_map<ol_program_handle_t, std::unique_ptr<ProgramWrapper>>
      MProgramWrappers;
  // All data collections, used and modified by kernel submissions, must be protected with this mutex.

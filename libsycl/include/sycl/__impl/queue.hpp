...
public:
  ...
  event parallel_for(nd_range<Dims> executionRange, Rest &&...rest) {
    return parallel_for<KernelName>(executionRange, {},
                                    std::forward<Rest>(rest)...);
  }
  ...
  event parallel_for(nd_range<Dims> executionRange, event depEvent,
                     Rest &&...rest) {
    return parallel_for<KernelName>(executionRange, {depEvent},
                                    std::forward<Rest>(rest)...);
  }
...
private:
  ...
  event parallelForImpl(Range<Dims> numWorkItems,
    ...
    static_assert(
        ...
        "Kernel argument of a sycl::parallel_for must be either sycl::nd_item, "
        "sycl::item or be convertible from sycl::item");
